# Clock Face and Sizing
clock_style = 'standard'
clock_face = False
clock_face_edge_color = 'white'
clock_face_face_color = 'none'
clock_face_line_width = 2

# Quarter Hour Markers
add_quarter_hour_markers = True
quarter_hour_line_color = 'black'
quarter_hour_line_length = .09
quarter_hour_line_width = .06
quarter_hour_line_offset = 0

# 5 Minute Markers
add_5_minute_markers = True
five_minute_color = 'black'
five_minute_line_length = .05
five_minute_line_width = .04
five_minute_line_offset = 0

# 1 Minute Markers
add_minute_markers = False
minute_line_color = 'white'
minute_line_length = 0.05
minute_line_width = 0.25
minute_line_offset = 0.95

# Hands
hand_color = 'black'
hour_hand_length = .3
hour_hand_width = .09
minute_hand_length = .44
minute_hand_width = .06

# Layout and Size
units = 'inches'  # options are inches, feet, cm, or mm
grid_rows, grid_cols = 24, 30
clock_diameter = 1.15
spacing = .35
outside_spacing = .7
background_color = 'white'
randomize_clocks = False

# Tiles for Matrix Mapping
leds_per_tile = 3
